# Jevon R., Multlying and divide fraction, 05/08/2019, version 0.6

print("Hello, Welcome to Fraction-Bot 9000.  I can multiply and divide fraction for you!\n")
user_name = input("What is your name? [Type your name and press ENTER.]\n")
print ("Hello,", user_name,"how are you today?\n")

print("For this program, I need to know the numerator and the denominator for both fractions.\n")

# Variable for fraction 0
numerator0 = 0
denominator0 = 0
print("The first fraction is",numerator0,"/",denominator0,".\n")

# Variable for fraction 1
numerator1 = 0
denominator1 = 0 
print("The second fraction is",numerator0,"/",denominator0,".\n")

print("When multiplying fractions you multiply the two numerator together.\n") 
print("Then you will multiply the two denominator together.\n")

new_numerator - numerator0 * numerator1
new_denominator -denominator0 * denominator1

print("The new fraction is", new_numerator,"/", new_denominator,".\n")

#This is where the division of fraction will start.
pie = 22 / 7
print(pie)
print("To divide a fraction, you will multiply using the reciprocal or inverse of the second fraction.\n")